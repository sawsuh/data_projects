  ...
  stages = list(
    stage("dealCards",
          nature = list(
            ...
            natureMove("card1", 1:13),
            # Draw from remaining cards for player 2
            natureMove("card2", ~setdiff(1:13, card1))
          )
    ),
    stage("pl1CheckBet",
          player=1,
...
#          )
    ...
    stage("PayoffStage",
          ...
          compute=list(
            ...
            payoff_1 ~ is_true(cb1 == "check")*1*ifelse(card1 > card2,1,-1) +
              is_true(cb1=="bet")*1*(
                is_true(fc2=="call")*1*ifelse(card1 > card2,3,-3) +
                is_true(fc2=="fold")*1
              ),
            #payoff_2 ~ (winner == 2)*pot - gave2
            payoff_2 ~ is_true(cb1 == "check")*1*ifelse(card1 > card2,-1,1) +
              is_true(cb1=="bet")*1*(
                is_true(fc2=="call")*1*ifelse(card1 > card2,-3,3) +
                is_true(fc2=="fold")*-1
              )
