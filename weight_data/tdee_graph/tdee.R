...
model <- function(curr_width, weight_width, cut_off, up_to, desired_daily_change, w) {
  ...
  df_clean <- read.csv('~/data_projects/weight_data/data.csv')
  ...
  tdee = -const/beta - (alpha/beta)*df_clean$weight
  plot(tdee)
  abline(h=mean(tdee))
  ...
  ## do calculations
  coefs <- coef(la.eq, s=0.006)
  print(coefs)
  const <- unname(coefs[1])
  alpha <- 0
  beta <- unname(coefs[3])
  ...
  abline(h=mean(tdee))
  upper <- 0.02
  lambdas <- seq(0.001,upper,(upper-0.001)/100)
  cals_seq <- sapply(lambdas, function(l) {
    la.eq_temp <- glmnet(
      x = as.matrix(df_clean[,c('weight','cals')]),
      y = df_clean$weight_change,
      family="gaussian",
    ...
    tdee <- (-alpha*w - const)/beta
    return((desired_daily_change - const - alpha*w)/beta)
  })
  plot(lambdas, cals_seq)
  print(lambdas)
  print(cals_seq)
